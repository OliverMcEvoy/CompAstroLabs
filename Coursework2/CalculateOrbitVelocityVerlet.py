...
def calculate_orbit(objects, initial_time_step=3600, max_steps=8766):
      ...
      accelerations = {obj['name']: {'x': np.zeros(max_steps), 'y': np.zeros(max_steps), 'z': np.zeros(max_steps)} for obj in objects}
      ...
      for i in tqdm(range(max_steps - 1), desc="Calculating orbits", mininterval=1.0):
            ...
            for obj in objects:
                  name = obj['name']
                  positions[name]['x'][i+1] = positions[name]['x'][i] + velocities[name]['x'][i] * time_step + 0.5 * accelerations[name]['x'][i] * time_step**2
                  positions[name]['y'][i+1] = positions[name]['y'][i] + velocities[name]['y'][i] * time_step + 0.5 * accelerations[name]['y'][i] * time_step**2
                  positions[name]['z'][i+1] = positions[name]['z'][i] + velocities[name]['z'][i] * time_step + 0.5 * accelerations[name]['z'][i] * time_step**2
            ...
            for obj in objects:
                  ...
                  velocities[name]['x'][i+1] = velocities[name]['x'][i] + 0.5 * (accelerations[name]['x'][i] + ax_new) * time_step
                  velocities[name]['y'][i+1] = velocities[name]['y'][i] + 0.5 * (accelerations[name]['y'][i] + ay_new) * time_step
                  velocities[name]['z'][i+1] = velocities[name]['z'][i] + 0.5 * (accelerations[name]['z'][i] + az_new) * time_step
