...
from AnimateOrbit import animate_orbit
import datetime
import random
...
def plot_orbit(positions, formatting, img_name, resolution=1000, just_3d=False):
    ...
    for name, pos in positions.items():
        ...
        if len(pos["x"]) > resolution:
            ...
            pos_slice = (
                pos  # If there are fewer than 1000 points, use all available points
            )

        fmt = formatting.get(
            name, {"linewidth": 0.5, "alpha": 0.75, "color": "black"}
        )  # Default formatting if for whatever reason none is passed in.
...
def calculate_orbital_periods(crossings, bootstrapping):
    ...
    for name, crossings in crossings.items():
        if len(crossings) > 1:
            ...
            half_periods = [
                crossings[i + 1] - crossings[i] for i in range(len(crossings) - 1)
            ]

            # Convert half-periods from seconds to full periods in years.
            period_in_years = [
                period * 2 / (365.25 * 24 * 3600) for period in half_periods
            ]
...
def calculate_closest_and_furthest_approach_between_earth_and_halleys_comet(
    positions, times, bootstrapping
):
    if "Earth" in positions and "Halleys_Comet" in positions:
        ...
        distances = (
            (earth_pos["x"] - comet_pos["x"]) ** 2
            + (earth_pos["y"] - comet_pos["y"]) ** 2
            + (earth_pos["z"] - comet_pos["z"]) ** 2
        ) ** 0.5
        ...
        print(
            f"Closest Approach: {closest_approach:.2e} AU at {closest_time:.2f} years, "
...
def main(
    ...
    start_distance=0,
    collision_object="solar_system",
):
    masses, objects_info, formatting = get_masses_and_object_info(example)

    # Load collision object information if start distance > 0
    if start_distance > 0:
        collision_mass, collision_objects_info, collision_formatting = (
            get_masses_and_object_info(collision_object)
        )
    else:
        collision_mass, collision_objects_info, collision_formatting = None, [], {}

    au = 1.496e11  # 1 Astronomical Unit in meters.
    day_in_seconds = 24 * 3600  # Number of seconds in a day.

    csv_file = "objects_data.csv"
    if os.path.exists(csv_file) and not bootstrapping:
        print("Loading data from CSV. Use --refresh to rewrite the csv.")
        objects = pd.read_csv(csv_file).to_dict(orient="records")
    else:
        start_date, end_date = (
            ("2024-03-17", "2024-03-18")
            if not bootstrapping
            else random_past_date(5 * 365)
        )
        objects = []
        ...
        # Add collision object data
        if start_distance > 0:
            for obj in collision_objects_info:
                horizons_obj = Horizons(
                    id=obj["id"],
                    location="500@10",
                    epochs={"start": start_date, "stop": end_date, "step": "1d"},
                ...
                obj_data = {
                    ...
                    "mass": collision_mass[obj["name"]],
                    "initial_x": eph["x"][0] * au,
                    "initial_y": eph["y"][0] * au + start_distance * au,
                    "initial_z": eph["z"][0] * au,
                    "initial_x_velocity": -eph["vx"][0] * (au / day_in_seconds),
                    "initial_y_velocity": -eph["vy"][0] * (au / day_in_seconds)
                    - collision_velocity,
    ...
    if start_distance > 0:
        print(
            f"Simulating collision between {example} and {collision_object} at {collision_velocity} m/s"
        )

    if method == "both" or method == "com":
        # Do the calculations to process the system
        # TODO rename _rk to com for vraibles too
        positions_rk, time, y_crossings_rk, radial_velocity_changes_rk = (
            calculate_orbit_com(objects, time_step, total_steps)
        )
        calculate_orbital_periods(y_crossings_rk, bootstrapping)
        rk_avgerage_periods, rk_halley_periods = calculate_orbital_periods(
            radial_velocity_changes_rk, bootstrapping
        )
        rk_earth_halley_appraoches = (
            calculate_closest_and_furthest_approach_between_earth_and_halleys_comet(
                positions_rk, time, bootstrapping
            )
        )

    if method == "both" or method == "vv":
        # Do the calculations to process the system
        positions_euler, time, y_crossings_euler, radial_velocity_changes_euler = (
            calculate_orbit(objects, time_step, total_steps)
        )

        calculate_orbital_periods(y_crossings_euler, bootstrapping)

        vv_average_periods, vv_halley_periods = calculate_orbital_periods(
            radial_velocity_changes_euler, bootstrapping
        )
        vv_earth_halley_appraoches = (
            calculate_closest_and_furthest_approach_between_earth_and_halleys_comet(
                positions_euler, time, bootstrapping
            )
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Simulate solar system")
    parser.add_argument("--refresh", action="store_true", help="Delete current CSV ")
    parser.add_argument(
        "--single_graph", action="store_true", help="Just a single pretty 3D grpah "
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Have 2 solar systems collide ",
    )
    parser.add_argument(
        "--start_distance_au",
        type=int,
        default=0,
        help="start distance between the 2 systems",
    )
    parser.add_argument(
        "--example",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="solar_system",
        help="collision example to collide with example 1",
    )
    ...
    if args.bootstrap_count > 0 and args.thread_count < 0:
        raise Exception(
            " When using bootstrapping please ensure the amount of threads the process will run on is specified. do this with the --thread_count flag e.g --thread_count 4 (if your unsure what this means just go for 8 and if your computer chugs reduce this as needed)"
        )
