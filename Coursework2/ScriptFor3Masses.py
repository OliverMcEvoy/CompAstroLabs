...
# Ill preface that this file is not the finest code I have ever written, it will do just to get this working and tested so I can spend more time on the cool stuff
...
objects.append( {
      ...
      'initial_y':0,
      'initial_x_velocity': 0,
      'initial_y_velocity': intial_velocity(2.52 * au,),
})
...
objects.append({
      ...
      'initial_y':0,
      'initial_x_velocity': 0,
      'initial_y_velocity': -intial_velocity(5.24 * au,),
})
      

# Initialize position and velocity arrays
max_steps = 20000000 + 1
positions = {obj['name']: {'x': np.zeros(max_steps), 'y': np.zeros(max_steps)} for obj in objects}
velocities = {obj['name']: {'x': np.zeros(max_steps), 'y': np.zeros(max_steps)} for obj in objects}
...
for obj in objects:
      name = obj['name']
      #This mehtod will have a couple of thousand points jsut for the sun, oh well. This code is just to get a graph produced for the report, I'll use the more important and significant pieces of code as demonstration I can code?
      x = positions[name]['x']
      y = positions[name]['y']

      plt.plot(x, y, label=f'Object {name}',linewidth = 1 )
