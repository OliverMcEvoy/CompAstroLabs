...
# It gets the meand and std for a couple of different things in a dictionary
...
def print_stats(stat_dict, label, unit=""):
    for key, (mean, std) in stat_dict.items():
        print(f"{key} & {mean:.4f} {unit} & ±{std:.4f} {unit}")


# This function has to take into account so many different cases, I think it handels it well.
# While its not the best, com will also be reffered to as rk here, and is com replaces the rk orbit. Lots of refactoring needed to fix this and its not too neccessary imo.
def analyse_bootstrap_results(bootstrap_results):
    ...
    for entry in bootstrap_results:
        ...
        (
            ...
            obj_dict,
            halley_period_list,
            earth_and_halley_approaches,
            obj_dict_if_both,
            halley_period_list_if_both,
            earth_and_halley_period_list_if_both,
        ...
        time_values["closest_time"].append(
            earth_and_halley_approaches[1] + year_difference
        )
        time_values["furthest_time"].append(
            earth_and_halley_approaches[3] + year_difference
        )
        ...
        if earth_and_halley_period_list_if_both:
            additional_approach_values["closest_approach"].append(
                earth_and_halley_period_list_if_both[0]
            )
            additional_approach_values["furthest_approach"].append(
                earth_and_halley_period_list_if_both[2]
            )
            additional_time_values["closest_time"].append(
                earth_and_halley_period_list_if_both[1] + year_difference
            )
            additional_time_values["furthest_time"].append(
                earth_and_halley_period_list_if_both[3] + year_difference
            )
    ...
    stats = {
        ...
        "additional_approach_stats": (
            compute_stats(additional_approach_values)
            if additional_approach_values["closest_approach"]
            else {}
        ),
        "additional_time_stats": (
            compute_stats(additional_time_values)
            if additional_time_values["closest_time"]
            else {}
        ),
    }
