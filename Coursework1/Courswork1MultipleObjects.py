...
def plot_orbit(positions, formatting, img_name, resolution=1000, just_3d=False):
    ...
    # Ensure the z doesnt look too weird ( NOTE: it always looks weird)
...
def main(
    ...
    if os.path.exists(csv_file) and bootstrapping == False:
        # Load data from CSV
        print(
            "CSV file already exists, loading data from CSV. NOTE if you have changed the example use the --refresh flag to rewrite the csv"
        )
    ...
    if method == "both" or method == "rk":
        ...
        positions_rk, time, y_crossings_rk, radial_velocity_changes_rk = (
            calculate_orbit_rk4(objects, time_step, total_steps)
        )
    ...
    if method == "both" or method == "rk":
        plot_orbit(
            positions_rk, formatting, output_img + "_rk", plot_count, three_d_graph_only
        )
    if method == "both" or method == "vv":
        plot_orbit(
            positions_euler,
