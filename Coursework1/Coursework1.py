...
def calculate_orbit(initial_radius, initial_velocity, initial_z_velocity, max_orbit_count, initial_time_step=1000, min_time_step=2000, max_steps=10000000, tolerance=1e-6):
    # Constants
    G = 6.67430e-11  
    star_mass = 1.989e30     
    comet_mass = 2.2e14       
    ...
    for i in range(0, max_steps-1):
        ...
        if i > min_points and x[i] > orbit_completion_tolerance:
            ...
            if orbit_completions == max_orbit_count:
                return x[:i+1], y[:i+1], z[:i+1], sun_z[:i+1], time[:i+1], orbit_completions
        ...
        sun_z[i+1] = sun_z[i] + z_velocity * time_step  # Update Sun's z-position
...
def plot_orbit(x, y, z, sun_z):
    ...
    ax.set_zlabel('z (m)')
    ax.legend()
    plt.savefig('orbit_plot_3d.png')  # Save the plot as an image file
    plt.show()
...
def main():
    ...
    # Run simulation
    x, y, z, sun_z, time, orbit_completions = calculate_orbit(initial_radius, initial_velocity, initial_z_velocity, max_orbit_count)
    plot_orbit(x, y, z, sun_z)

    # Calculate orbital period
    period = time[-1] / (365.25 * 24 * 3600 * orbit_completions)  # Convert to years and divide by number of orbits
